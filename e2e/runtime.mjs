...
const configuredRunDir = process.env.NEWS_E2E_RUN_DIR;
export const port = Number(process.env.NEWS_E2E_PORT);
if (!configuredRunDir || basename(dirname(configuredRunDir)) !== '.playwright' || !basename(configuredRunDir).startsWith('run-') || !Number.isInteger(port) || port < 1 || port > 65535) {
  throw new Error('Run browser tests with npm run test:e2e (isolated state and port required).');
}
export const runDir = configuredRunDir;
